...
# Loto 7/39 (tvoj CSV sa headerom Num1..Num7)
MIN_NUMBER = 1
MAX_NUMBER = 39
NUMBERS_PER_DRAW = 7
...
# Isti CSV + isti seed → isti portfolio pri svakom pokretanju
RNG_SEED = int(os.getenv("LOTTO_RNG_SEED", "39"))

# Models directory
MODELS_DIR = Path(__file__).resolve().parent / "models"
MODELS_DIR.mkdir(parents=True, exist_ok=True)

# Scraping configuration (isključeno — koristi se CSV)
...
logger.info(f"Number range: {NUMBER_RANGE}, numbers per draw: {NUMBERS_PER_DRAW}")
logger.info(f"Scraping enabled: {SCRAPING_ENABLED}")
logger.info(f"RNG seed (portfolio): {RNG_SEED}")
